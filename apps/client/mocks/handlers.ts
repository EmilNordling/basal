...
import { type FilesModel } from '@application/internal/api/models/files_model';
import { type UserModel } from '@application/internal/api/models/user_model';
import { type FileDetailsModel } from '@application/internal/api/models/file_details_model';
import { LogBooksModel } from '@application/internal/api/models/logbook_model';
...
const db = {
  ...
  eventListTable: [
    ...
    { id: (1).toString(), fileId: (0).toString(), event: 'Leauge of Legends RP' },
  ],
...
export const handlers = [
  ...
  http.get('api/file/:id', async ({ cookies, params }) => {
    ...
    const details = db.fileDetailsTable.find((file) => file.fileId === params.id);
    if (details == null) {
      return new HttpResponse(null, { status: 404 });
    }
  ...
  http.get('api/file/:fileId/logbook', async ({ cookies, params }) => {
    ...
    const list = db.eventListTable.filter((file) => file.fileId === params.fileId);
